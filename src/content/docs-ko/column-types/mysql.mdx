...
### binary
`BINARY(M)`은 정확히 M바이트 길이의 고정 길이 바이트 문자열을 저장합니다.  
삽입 시 더 짧은 값은 `0x00` 바이트로 오른쪽 패딩되어 M바이트가 되며, 조회 시에는 패딩이 제거되지 않습니다.
후행 `0x00`을 포함한 모든 바이트는 비교, `ORDER BY`, `DISTINCT`에서 모두 의미를 가집니다.
<Section>
```typescript
...
### varbinary
`VARBINARY(M)`은 정확히 M바이트 길이의 가변 길이 바이트 문자열을 저장합니다.  
삽입 시 더 짧은 값은 `0x00` 바이트로 오른쪽 패딩되어 M바이트가 되며, 조회 시에는 패딩이 제거되지 않습니다.
후행 `0x00`을 포함한 모든 바이트는 비교, `ORDER BY`, `DISTINCT`에서 모두 의미를 가집니다.
<Section>
```typescript
...
`drizzle-orm@1.0.0-beta.2`부터 사용할 수 있습니다.
</Callout>

`BLOB`은 가변적인 양의 데이터를 저장할 수 있는 바이너리 대형 객체입니다.
<Section>
```typescript
...
`TINYBLOB`은 가변적인 양의 데이터를 저장할 수 있는 바이너리 대형 객체입니다.
<Section>
```typescript
...
`MEDIUMBLOB`은 가변적인 양의 데이터를 저장할 수 있는 바이너리 대형 객체입니다.
<Section>
```typescript
...
`LONGBLOB`은 가변적인 양의 데이터를 저장할 수 있는 바이너리 대형 객체입니다.
<Section>
```typescript
