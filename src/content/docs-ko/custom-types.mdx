...
# custom type를 정의하는 일반적인 방법

## 예시

`customType` 정의가 어떻게 동작하는지 확인하는 가장 좋은 방법은
기존 데이터 타입이 Drizzle ORM의 `customType` 함수를 사용해 어떻게 정의될 수 있는지 살펴보는 것입니다.


<Callout title='정보'>
각 dialect는 `customType` 함수를 노출합니다
...
모든 타입의 사용법은 Drizzle ORM에 정의된 함수들과 동일합니다. 예를 들면:
...
`types`와 `param` 정의에 대한 ts-doc를 확인할 수 있습니다.
