...
<Callout type="error">
`drizzle-orm@1.0.0-beta.15`부터 `drizzle-valibot`는 Drizzle ORM 자체에 포함된 기본 제공 스키마 생성 지원으로 대체되어 사용 중단되었습니다.

여전히 `drizzle-valibot` 패키지를 사용할 수는 있지만, 앞으로의 새로운 업데이트는 모두 Drizzle ORM에 직접 추가됩니다.
</Callout>
...
각 create schema 함수는 필드의 스키마를 확장하거나 수정하거나 완전히 덮어쓸 수 있는 추가적인 선택적 파라미터를 받습니다. 콜백 함수를 정의하면 스키마를 확장하거나 수정하고, Valibot 스키마를 직접 제공하면 해당 필드를 덮어씁니다.
