...
# Filter 및 conditional operator
모든 dialect별 filter 및 conditional operator를 기본으로 지원합니다.

모든 filter와 conditional을 `drizzle-orm`에서 import할 수 있습니다:
```typescript copy
...
값이 다른 값과 like 비교되며 대소문자를 구분합니다
<Section>
```typescript
...
값이 다른 값과 like 비교되며 대소문자를 구분하지 않습니다
<Section>
```typescript
...
값이 다른 값과 like 비교되지 않으며 대소문자를 구분하지 않습니다
<Section>
```typescript
...
column 또는 expression이 두 번째 인자로 전달된 목록의 모든 요소를 포함하는지 검사합니다
...
두 번째 인자로 전달된 목록이 column 또는 expression의 모든 요소를 포함하는지 검사합니다
...
column 또는 expression이 두 번째 인자로 전달된 목록의 요소 중 하나라도 포함하는지 검사합니다.
